...
async function waitForCSharpApi(timeout = CONFIG.API_TIMEOUT) {
  ...
  while (Date.now() - startTime < timeout) {
    if (window.csharpApi && typeof window.saveFile === "function") {
      try {
        await CefSharp.BindObjectAsync("csharpApi");
        console.log("✅ C# API 已就绪");
        return true;
      } catch (e) {
        // 忽略错误，继续等待
      }
...
async function saveCustomerDataToFile(customers) {
  try {
    ...
    if (result && result.success) {
      ...
      // 保存备份
      try {
        await window.saveFile(CONFIG.CUSTOMER_BACKUP_FILE, {
          ...dataToSave,
          metadata: {
            ...dataToSave.metadata,
            isBackup: true,
  ...
  try {
    ...
    if (data !== null) {
      ...
      if (Array.isArray(data)) {
        ...
        customers = Object.values(data).filter(
          (v) => v && (v.号码 || typeof v === "string"),
        );
...
async function 手动刷新客户数据() {
  ...
  if (loadResult.success && loadResult.customers.length > 0) {
    window.__客户号码列表 = new Set(
      loadResult.customers.map((c) => c.号码 || c),
    );
...
async function 查看客户文件信息() {
  try {
    ...
    if (data) {
      ...
      if (data.customers) {
        const sample = data.customers
          .slice(0, 5)
          .map((c) => c.号码)
          .join(", ");
        console.log(
          `   客户列表: ${sample}${data.customers.length > 5 ? "..." : ""}`,
        );
      }
    } else {
      console.log(`📂 文件不存在: ${CONFIG.CUSTOMER_DATA_FILE}`);
    }
  } catch (error) {
...
async function 标记客户(开启 = true) {
  ...
  if (开启) {
    ...
    try {
      ...
      if (loadResult.success && loadResult.customers.length > 0) {
        console.log(
          `📚 已加载 ${loadResult.customers.length} 个客户号码 (来源: ${loadResult.source})`,
        );
        window.__客户号码列表 = new Set(
          loadResult.customers.map((c) => c.号码 || c),
        );
        ...
        console.log(
          `📋 示例客户: ${sample.join(", ")}${window.__客户号码列表.size > 3 ? "..." : ""}`,
        );
      } else {
  ...
  if (analysis.uniqueNumbers && analysis.uniqueNumbers.length > 0) {
    try {
      ...
      if (saveResult.success) {
        console.log(
          `📦 已将 ${analysis.uniqueNumbers.length} 个独立号码保存到 ${CONFIG.CUSTOMER_DATA_FILE}`,
        );
        if (saveResult.path) {
        ...
        window.__客户号码列表 = new Set(
          analysis.uniqueNumbers.map((item) => item.号码),
        );
      } else {
...
function 注入浮动窗口() {
  ...
  function getSelectedSendOption() {
    const selectedOption = shadowRoot.querySelector(
      '.send-options input[name="sendOption"]:checked',
    );
...
console.log("🚀 WhatsApp 群发助手已加载完成！");
